...
export class ExternalLoginValueConverter {
     ...
     */
    toView(authority) {
        let url = `${window.location.origin}/Accounts/ExternalLogin?tenant=${authority.tenant}&application=${authority.application}&authority=${authority.type}&authority=${authority.id}&${window.location.search.substr(1)}`;
        return url;
    }
}